// Minha Floresta Backend API - Clean Deployment
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (path === '/' && method === 'GET') {
      return new Response(
        JSON.stringify({
          message: 'Minha Floresta Backend API',
          status: 'operational',
          version: '1.0.0',
    ...
    // Projects endpoints (Postgres)
    if (path === '/projects' && method === 'GET') {
      const { data, error } = await supabaseClient
        .from('projects')
        .select('id, name, description, location, price_per_sqm, available_area, sold_area, image, type, status');
      if (error) {
        return new Response(
      ...
      const mapped = (data || []).map((p: any) => ({
        ...
        location: p.location,
        price: p.price_per_sqm,
        available: p.available_area,
        sold: p.sold_area || 0,
        image: p.image,
        type: p.type,
        status: p.status,
    ...
    // Admin endpoints
    if (path === '/admin/seed-projects-postgres' && method === 'POST') {
      ...
      try {
        ...
        const sample = [
          {
            name: 'Amazônia Verde',
            description: 'Área de conservação certificada',
            location: 'Amazonas - AM',
            price_per_sqm: 25.0,
            available_area: 50000,
            type: 'conservation',
            status: 'active',
            image: 'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=1600',
          },
          {
            name: 'Restauro Atlântico',
            description: 'Restauração de Mata Atlântica',
            location: 'Vale do Ribeira - SP',
            price_per_sqm: 15.0,
            available_area: 30000,
            type: 'restoration',
            status: 'active',
            image: 'https://images.unsplash.com/photo-1569163139394-de4e4f43e4e3?w=1600',
          },
          {
            name: 'Blue Carbon Manguezal',
            description: 'Proteção de manguezais (Blue Carbon)',
            location: 'Ilha do Marajó - PA',
            price_per_sqm: 12.0,
            available_area: 40000,
            type: 'blue-carbon',
            status: 'active',
            image: 'https://images.unsplash.com/photo-1574263867128-ca4c7707e1c8?w=1600',
          },
        ];

        const { data, error } = await supabaseClient.from('projects').insert(sample).select('id, name');
        if (error) {
        ...
        return new Response(
          JSON.stringify({ success: true, inserted: data?.length ?? 0, projects: data }),
    ...
    if (path.match(/^\/certificates\/[a-zA-Z0-9-]+$/) && method === 'GET') {
      const code = path.split('/')[2];
      const certificate = await getKVData(`certificate_${code}`);
    ...
    if (path === '/certificates' && method === 'POST') {
      ...
      await setKVData(`certificate_${code}`, { 
        ...certificate, 
        code, 
        timestamp: new Date().toISOString() 
      });
    ...
    // Calculator endpoint
    if (path === '/calculator' && method === 'POST') {
      const data = await req.json();
      const id = crypto.randomUUID();
      await setKVData(`calculation_${id}`, { 
        ...data, 
        id, 
        timestamp: new Date().toISOString() 
      });
      ...
      return new Response(
        JSON.stringify({ success: true, id, calculation: data }),
        {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
      );
