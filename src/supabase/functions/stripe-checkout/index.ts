    ...
    if (type === 'purchase') {
      ...
      for (const item of items) {
        const { data: project, error } = await supabase
          .from('projects')
          .select('available_area, price_per_sqm')
        ...
        if (project.available_area < item.quantity) {
          return new Response(
            JSON.stringify({ 
      ...
      // Criar registro de compra (status: pending)
      ...
      // Atualizar purchase com stripe_payment_intent_id
      await supabase
        .from('purchases')
