...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Donations endpoints
    if (cleanPath === '/donations' && method === 'GET') {
      const donations = await getKVByPrefix('donation_');
      return new Response(
    ...
    if (cleanPath === '/donations' && method === 'POST') {
    ...
    if (cleanPath.match(/^\/certificates\/[a-zA-Z0-9-]+$/) && method === 'GET') {
    ...
    if (cleanPath === '/certificates' && method === 'POST') {
    ...
    // Calculator endpoint
    if (cleanPath === '/calculator' && method === 'POST') {
